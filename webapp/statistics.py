...
#combine statistics
def surname_statistics(clean_sur):

    #statistics -- forename
    forenames_hist = names_forenames_hist.objects.filter(surname=clean_sur).values('forename','sex')
    forenames_cont = names_forenames_cont.objects.filter(surname=clean_sur).values('forename','sex')
