...
| `fpc` (new; was called precarity) | Did not exist. | **Financial Precarity Classification v2**: most common group (13, inside 5 clusters), joined on `lsoa21cd`. The source data is **safeguarded**, so the download and its lookup table are never in the repository, and it is still to be confirmed with its owner that a per-surname most common group may be shown. Not yet part of the fact list of the file format above. |
