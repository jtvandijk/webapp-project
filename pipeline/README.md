...
| `preview_web.py` | A quick look at already-assembled release files: a name's maps, bearers per year and its facts as small tables (OAC/LOAC/FPC groups by name, from `reference/group_names.json`), no database, no computation, stdlib only. | done, tested |
